...
GLSL_SRCS := $(shell find $(SRC_DIRS) -name *.glsl | grep -v $(MAIN_SRC_DIR))
...
RELEASE_OBJS := $(SUB_OBJS_RELEASE) obj/release/src/main/main.cpp.o
release: move_shaders $(RELEASE_OBJS)
	@$(MKDIR_P) $(dir $(RELEASE_EXEC))
	$(CXX) $(RELEASE_OBJS) -o $(RELEASE_EXEC) $(LD_FLAGS)

# Debug build target
DEBUG_OBJS := $(SUB_OBJS_DEBUG) obj/debug/src/main/main.cpp.o
